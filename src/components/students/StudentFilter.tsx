    ...
    <div className="bg-white p-4 sm:p-5 rounded-xl border border-slate-200 shadow-xs mb-6 print:hidden">
        ...
        <div className="flex flex-wrap sm:flex-nowrap items-center gap-3">
          ...
          {(searchTerm || selectedYear || selectedClass) && (
            <button
              ...
              onClick={onReset}
              title="Reset Filters"
              className="px-3 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-medium transition flex items-center gap-1.5 shrink-0 cursor-pointer"
            >
              <RotateCcw className="w-4 h-4" />
              <span className="hidden sm:inline">{language === 'mr' ? 'रीसेट' : 'Reset'}</span>
            </button>
          )}
        </div>
      </div>
      ...
      <div className="mt-3 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
        <span className="flex items-center gap-1.5 font-medium">
          ...
          {language === 'mr' ? (
            <span>एकूण <strong>{totalResults}</strong> विद्यार्थी नोंदी दाखवत आहे</span>
          ) : (
            <span>Showing <strong>{totalResults}</strong> student record{totalResults === 1 ? '' : 's'}</span>
          )}
        </span>
        {(searchTerm || selectedYear || selectedClass) && (
          <span className="text-blue-700 bg-blue-50 px-2 py-0.5 rounded text-[11px] font-semibold">
            {language === 'mr' ? 'फिल्टर केलेले' : 'Filtered View'}
          </span>
        )}
      </div>
    </div>
